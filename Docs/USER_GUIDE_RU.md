...
4. `Кэш зависимостей` (`wheelhouse`) входит в раздачу и показывается как `Установлен`. `Переустановить` нужно только если папку удалили: он заново скачает wheels для GigaAM/ONNX Runtime.
5. Live dependencies (микрофон, потоковая диктовка) GUI ставит сам при запуске из `wheelhouse\live`, без сети. Ручная кнопка остаётся для ремонта.
...
9. Studio: `Модель whisper.cpp Large V2` — основная модель для файлов в режиме CUDA. `Диаризация на GPU` (torch + pyannote Community-1) — по желанию, только на NVIDIA и только с ключом HuggingFace; окно первого запуска её не ставит.
10. `Разделение по говорящим (без ключа)`: sherpa-onnx и две модели, около 65 МБ, по желанию; окно первого запуска его не ставит, потому что разделение грубое.
11. Запустить verify/smoke-проверки базовых модулей.
...
Отдельно стоит необязательный **ключ HuggingFace** (`config\api_key_huggingface.txt`, строка «Ключ HuggingFace» на странице `Обслуживание`). Без него модели качаются анонимно, только медленнее; с ключом HuggingFace снимает ограничение скорости. Получить ключ за минуту: huggingface.co → войти или зарегистрироваться → аватар → Settings → Access Tokens → New token, тип Read → скопировать и вставить в строку ключа. В Studio тот же ключ открывает разделение по говорящим: этим же аккаунтом откройте страницу `pyannote/speaker-diarization-community-1` и примите условия модели.
...
**Разделение по говорящим** в локальных режимах включается карточкой `Разделение по говорящим` на вкладке `Файлы` (она же — развилка `Транскрипция + диаризация` в режиме API). Путь без ключа HuggingFace ставится модулем `Разделение по говорящим (без ключа)`: sherpa-onnx и две ONNX-модели, около 65 МБ, работает на процессоре и занимает примерно от пятой части до половины длительности записи. Этому движку нужно число говорящих в поле `Говорящих` (от 2 до 20). Автоопределения у него нет намеренно: у sherpa-onnx нет откалиброванного порога, на одной и той же записи он находит от одного до семи человек в зависимости от настройки, а подгонять программу под одну запись мы не стали. Без числа разделение не запускается, расшифровка сохраняется без меток, в журнале остаётся предупреждение. Разделение без ключа грубое: непохожие голоса оно отделяет, похожие может слить. На проверочной записи трёх человек в одной комнате оно совпало с Community-1 на трёх четвертях сегментов и двоих с похожими голосами приняло за одного; Community-1 на той же записи без подсказки нашёл всех троих.
...
2. Если нужна разметка по говорящим: ввести ключ HuggingFace на странице `Обслуживание`, принять условия модели `pyannote/speaker-diarization-community-1` и установить `Диаризация на GPU`. Без ключа установка не начнётся и объяснит почему.
3. Запустить verify.
...
- Если разделение по говорящим не сработало (нет pyannote, нет ключа HuggingFace, не приняты условия модели), расшифровка всё равно сохраняется, а в журнале появляется строка `WARN: speaker separation skipped` с причиной и подсказкой.
